...
public enum NetworkKitError: Error {
    ...
    /// The server returned an invalid response (not an HTTPURLResponse).
    ...
    case decodingFailed(response: HTTPResponse<Data>, underlyingError: Error)
}
...
extension NetworkKitError: LocalizedError {
    public var errorDescription: String? {
        ...
        case .decodingFailed(let response, let underlyingError):
            return "Failed to decode response (status \(response.statusCode)): \(underlyingError.localizedDescription)"
        }
    }
}
